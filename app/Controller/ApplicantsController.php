...
class ApplicantsController extends AppController {
	...
	public $uses = array('Authentication', 'Applicant');
		...
		if (isset($token)) {
			...
			if (!empty($applicant)) {
				if ($this->checkTokenValidity($applicant['Applicant']['token_validity'])
					...
					$this->Authentication->save(array(
						'date' => date("Y-m-d H:i:s"),
						'action' => 'LO',
						'connection_ip' => $this->request->clientIp(false),
...
public function signup() {
	...
	if ($this->request->is('post')) {
		$applicant = $this->Applicant->find('first', array(
			...
			'recursive' => -1));
		if (!empty($applicant)) {
			if ($applicant['Applicant']['token'] != null) {
				if ($this->checkTokenValidity($applicant['Applicant']['token_validity'])) {
					$token = $applicant['Applicant']['token'];
					$data = array(
